      ...
      for (const r of results) {
        try {
          ...
          if (result.rows.length > 0) {
            persistedCount++;
            console.log(`  ✓ Persisted match: ${r.teacherName} for ${r.skillTitle || skillName} (score: ${r.score})`);
          }
        } catch (err) {
          console.error(`  ✗ Failed to persist match for ${r.teacherName}:`, err.message);
        }
      }
      console.log(`✅ Successfully persisted ${persistedCount}/${results.length} matches to skill_matches table`);
    } catch (e) {
      console.error('⚠️ Failed to persist matches:', e.message, e.stack);
    }
  ...
  persistCycles: async (scoredCycles) => {
    ...
    for (const { cycle, score } of scoredCycles) {
      ...
      try {
        ...
        console.error('⚠️ Failed to persist cycle:', e.message);
      } finally {
        client.release();
      }
    }

    console.log(`✅ Persisted ${persistedCount.length} cycles to database`);
